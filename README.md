...
- **読み上げページ**（`read.html`）：**タグも注釈も無い、台本の本文だけ。**iPhone の Safari など、**ブラウザ側の読み上げ機能で聴くため**のページ
  - **間を置くところが、そのまま改行**になっている（読み上げは段落の切れ目で息を入れるため）
  - **このページだけ、リンクとタブが最下部にある。**読み上げは画面を上から読むので、上にあると本文の前にボタン名が読まれてしまう。並びは「タイトル → 台本 → リンク類」
  - 元は `script_tts.txt` なので**読み辞書の読みが当たった状態**で読まれる（目で追うと仮名が多いのはそのため）
